...
infowrapFilepicker.provider("fpConfig", function() {
  var _apiKey, _debug, _options, _security;
  _apiKey = void 0;
  _debug = false;
  _security = false;
  _options = {};
  return {
    ...
    setSecurity: function(enable) {
      return _security = enable;
    },
...
infowrapFilepicker.provider("infowrapFilepickerService", function() {
  return {
    $get: [
      "fpConfig", "infowrapFilepickerSecurity", "$log", "$window", "$q", "$rootScope", "$timeout", function(config, fps, $log, $window, $q, $rootScope, $timeout) {
...
infowrapFilepicker.provider("infowrapFilepickerSecurity", function() {
  return {
    $get: [
      "fpConfig", "$log", "$q", "$rootScope", "$http", function(config, $log, $q, $rootScope, $http) {
          ...
          if (!signingInProcess) {
            ...
            handleError = function(error) {
              ...
              if (config.options().errorHandling) {
                _ref = config.options().errorHandling;
                _results = [];
            ...
            $http.post(config.options().signApiUrl(opt.resourceId, opt.signType), signage).success(function(result) {
...
infowrapFilepicker.run([
  'fpConfig', 'infowrapFilepickerService', '$rootScope', '$window', function(config, fp, $rootScope, $window) {
    if (config.apiKey()) {
...
infowrapFilepicker.directive("filepickerBtn", [
  "fpConfig", "infowrapFilepickerService", "infowrapFilepickerSecurity", "$timeout", "$rootScope", function(config, fp, fps, $timeout, $rootScope) {
    var link;
