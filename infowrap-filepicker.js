...
infowrapFilepicker.factory("infowrapFilepickerService", [
  "infowrapFilepicker.config", "infowrapFilepickerSecurity", "$log", "$window", "$q", "$rootScope", "$timeout", function(config, fps, $log, $window, $q, $rootScope, $timeout) {
    var allEvents, api, apiMethods, defaultPickOptions, helperEvents, pickOptions, preparePickOptions;
    ...
    */
    ...
    api.positionModal = function() {
      var $iframeContainer, height, left, positionSettings, screenSize, top, width;
    ...
    */
    ...
    api.modalToggle = function(force) {
      ...
      enabled = _.isNothing(force) ? !$rootScope.filepickerModalOpen : force;
      ...
      if (enabled) {
        ...
        }, BizBuilt.platform.IS_MOBILE ? 300 : 0);
      } else {
...
infowrapFilepicker.factory("infowrapFilepickerSecurity", [
  "infowrapFilepicker.config", "infowrapFilepickerService", "$q", "$rootScope", "$http", function(config, fp, $q, $rootScope, $http) {
    ...
    api.sign = function(opt) {
      ...
      if (!signingInProcess) {
        ...
        $http.post("" + BizBuilt.uri.apiServer + "wraps/" + opt.projectId + "/file_assets/sign.json", signage).success(function(result) {
          signingInProcess = false;
...
infowrapFilepicker.run([
  "infowrapFilepicker.config", 'infowrapFilepickerService', '$rootScope', function(config, fp, $rootScope) {
    if (config.apiKey) {
    ...
    return $rootScope.$on(BizBuilt.scope.events.modal.backdropclick, function() {
      return fp.modalToggle(false);
    });
  }
]);
      ...
      processFiles = function(fpfiles) {
        ...
        if (scope.previewOnUpload) {
          ...
          if (!scope.preventDefault) {
            ...
            if (scope.ignoreReadSigning === 'true') {
